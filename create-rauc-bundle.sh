...
mkdir -p ~/rauc_bundle_workspace
cd ~/rauc_bundle_workspace || { echo "ERROR: Failed to change to workspace directory."; exit 1; }
...
mkdir -p /mnt/source_systemA
mount -o ro /dev/sda2 /mnt/source_systemA || { echo "ERROR: Failed to mount /dev/sda2. Ensure it's not mounted elsewhere."; exit 1; }
...
mksquashfs /mnt/source_systemA rootfs_systemA.squashfs -comp xz || { echo "ERROR: Failed to create SquashFS image."; exit 1; }
